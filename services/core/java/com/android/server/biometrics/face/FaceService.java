    ...
    private IBiometricsFaceClientCallback mDaemonCallback =
        ...
        public void onLockoutChanged(long duration) {
            ...
            if (!isScreenOn(getContext())) {
                Slog.d(TAG, "Received lockout with screen off! Current count is {count}".replace("{count}", String.valueOf(mCurrentScreenOffLockouts)));
                mCurrentScreenOffLockouts++;
            } else {
                Slog.d(TAG, "Received lockout with screen on!");
            }

            mHandler.post(() -> {
                if (duration == 0
                    || mCurrentScreenOffLockouts >= MAX_SCREEN_OFF_LOCKOUTS) {
                    Slog.d(TAG, "Resetting locksouts!");
